...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            if not ret_val:
                ...
                if self.count == self.nf:  # last video
                    ...
                    self.new_video(path)
                    ret_val, img0 = self.cap.read()
            if self.frame + 15 < self.nframes:
                self.frame += 15
                for i in range(15):
                    self.cap.read()
            else:
                self.frame += 1
            #print(f'video {self.count + 1}/{self.nf} ({self.frame}/{self.nframes}) {path}: ', end='', flush=True)
        else:
            # Read image
            self.count += 1
        ...
        log_output = f'{{"frame": "({self.frame}/{self.nframes})",'
        return path, img, img0, self.cap, log_output
